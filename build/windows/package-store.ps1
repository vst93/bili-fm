...
# Tauri builds the raw executable into src-tauri/target/<triple>/release/ (or target/release)
if ($ExeDir) {
  $binDir = $ExeDir
} else {
  $binDir = Join-Path $workspace "src-tauri/target/release"
}
$exeName = "$DisplayName.exe"
$exePath = Join-Path $binDir $exeName
$templatePath = Join-Path $PSScriptRoot "AppxManifest.xml.template"
$manifestPath = Join-Path $packageRoot "AppxManifest.xml"
# Tauri icon source (512x512 recommended for logo generation)
$sourceIcon = Join-Path $workspace "src-tauri/icons/icon.png"
$msixPath = Join-Path $buildRoot "msix/$DisplayName-windows-amd64.msix"

# MSIX requires a 4-part version (a.b.c.d). Auto-fill the 4th part if a 3-part
# version is provided (e.g. Tauri's "2.0.0" -> "2.0.0.0").
if (-not $Version) {
